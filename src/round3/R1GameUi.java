...
public class R1GameUi extends JFrame implements ActionListener, MouseListener, MouseMotionListener {
	...
	public R1GameUi(Object obj) {
		main = (Main) obj;
		f1 = new Font("고딕", Font.BOLD, 40);
		f2 = new Font("고딕", Font.BOLD, 30);
		boomL = new JLabel[10];
		...
		jt1 = new JTextField("성 체력 : " + castleHp);
		jt2 = new JTextField("소지 금액 : " + money);
		jt3 = new JTextField("남은 몬스터 : " + spawn);
		jt4 = new JTextField("현재 점수 : " + score);
		...
		if (gamecnt == 0) {
			...
			for (int i = 0; i < maxSpawn; i++) {
				...
				if (dis < disxy) {
					...
					eneC[i].atkHp(dmg);
					System.out.println("남은체력 : " + eneC[i].getHp());
					if (disxy == 300) {
...
//		System.out.println(jbhero.getIcon());
	...
	public void update() {
		...
		Main.money = money;
		Main.score += score;
		System.out.println("메인" + Main.score);
		System.out.println("로컬" + score);
	}
